 ...
 *   -> "joseph leblanc"
 */
function normalizeNameWithoutMiddleInitials(name) {
  ...
  const filteredMiddle = middle.filter((part) => !/^[a-z]\.?$/.test(part));

  return [first, ...filteredMiddle, last].join(" ");
}
...
function getNameParts(name) {
  ...
  if (normalized.includes(",")) {
    const [lastPart, firstPart] = normalized
      .split(",")
      .map((part) => part.trim());

    return {
      first: firstPart?.split(" ")[0]?.replace(/\./g, "") || "",
      last: lastPart?.replace(/\./g, "").trim() || "",
    };
  }
  ...
  const withoutMiddle = normalizeNameWithoutMiddleInitials(normalized);
  ...
  const parts = withoutMiddle.split(" ");
...
console.log("Cameron, E.M. →", getNameParts("Cameron, E.M."));

console.log("Cameron, E. →", getNameParts("Cameron, E."));

console.log("Erin Cameron →", getNameParts("Erin Cameron"));

console.log("Erin C. Cameron →", getNameParts("Erin C. Cameron"));
...
const TEAM_BY_NAME_NO_MIDDLE = new Map();

for (const member of TEAM) {
  const normalized = normalizeName(member.name);

  const withoutMiddle = normalizeNameWithoutMiddleInitials(member.name);

  TEAM_BY_NAME.set(normalized, member);

  TEAM_BY_NAME_NO_MIDDLE.set(withoutMiddle, member);
}
...
function findTeamMemberByName(name) {
  ...
  const exact = TEAM_BY_NAME.get(normalized);
  ...
  const withoutMiddle = normalizeNameWithoutMiddleInitials(name);

  const noMiddle = TEAM_BY_NAME_NO_MIDDLE.get(withoutMiddle);
  ...
  const contributorParts = getNameParts(name);

  if (!contributorParts.first || !contributorParts.last) {
    return null;
  }

  const contributorFirst = contributorParts.first;

  const contributorLast = contributorParts.last;
  // ----------------------------------------------------------
  ...
  const nameParts = normalized.replace(/\./g, "").split(/\s+/);
  ...
  if (
    ...
    const possibleLast = nameParts[0];

    const initials = nameParts[1];

    const initialMatches = TEAM.filter((member) => {
      const memberParts = getNameParts(member.name);

      return (
        memberParts.last === possibleLast &&
        memberParts.first.charAt(0) === initials.charAt(0)
      );
    });
  ...
  const contributorInitials = contributorFirst.replace(/[^a-z]/g, "");

  const isInitialFormat =
    contributorInitials.length <= 2 && contributorInitials.length > 0;

  if (isInitialFormat) {
    const initialMatches = TEAM.filter((member) => {
      const memberParts = getNameParts(member.name);

      if (memberParts.last !== contributorLast) {
        return false;
      }

      const memberFirstInitial = memberParts.first.charAt(0);

      return contributorInitials.charAt(0) === memberFirstInitial;
    });
    ...
    if (initialMatches.length > 1) {
      console.log(`    ⚠️ Ambiguous author initials "${name}" — not matched`);
  ...
  for (const member of TEAM) {
    ...
    if (contributorLast !== memberParts.last) {
      continue;
    }

    const firstDistance = levenshtein(contributorFirst, memberParts.first);
...
function identifyContributor(contributor) {
  ...
  if (orcid) {
    const teamMember = TEAM.find(
      (member) => member.orcid.toLowerCase() === orcid,
    );
  ...
  if (name) {
    ...
    const teamMember = findTeamMemberByName(name);
...
function addCollaboration(collaborationMap, personA, personB, work) {
  ...
  const alreadyAdded = link.works.some(
    (existing) => existing.putCode === work.putCode,
  );
...
async function buildUnifiedGraph() {
  ...
  for (const member of TEAM) {
    ...
    try {
      ...
      for (const workGroup of works) {
        ...
        if (!summary) {
          console.log("  ⚠ Skipping work group with no summary");
        ...
        if (putCode === undefined || putCode === null) {
          console.log("  ⚠ Skipping work with no put-code");
        ...
        console.log(`  Work ${putCode}: "${title}"`);

        console.log(`    Contributors from ORCID: ${contributors.length}`);

        for (const contributor of contributors) {
          const person = identifyContributor(contributor);

          if (!person.id || !person.name) {
            console.log("    ⚠ Skipped contributor with missing ID/name");
          ...
          const alreadyInPeople = people.some(
        ...
        console.log(`    Canonical participants: ${people.length}`);

        // ----------------------------------------------------
        // Print useful debugging information.
        // ----------------------------------------------------

        console.log(`    Owner: ${ownerPerson.name} → ${ownerPerson.id}`);

        for (const person of people) {
          console.log(
            `    Author: ${person.name} → ${person.id}${
              person.teamMember ? " [CORE]" : " [EXTERNAL]"
            }`,
          );
        }
        ...
        for (let i = 0; i < people.length; i++) {
          for (let j = i + 1; j < people.length; j++) {
            addCollaboration(collaborationMap, people[i], people[j], work);
      ...
      console.log(`    Works saved: ${savedWorks}`);

      console.log(`    Collaboration pairs generated: ${collaborationPairs}`);
    } catch (error) {
      console.error(
  ...
  console.log(`Works: ${output.works.length}`);
  ...
  for (const member of TEAM) {
    const count = allWorks.filter(
      (work) => work.owner?.orcid === member.orcid,
    ).length;

    console.log(`  ${member.name}: ${count}`);
  }
  ...
  for (const link of links) {
    const sourceIsCore = TEAM.some((member) => member.orcid === link.source);

    const targetIsCore = TEAM.some((member) => member.orcid === link.target);
...
buildUnifiedGraph().catch((error) => {
  console.error("\n✗ Fatal error while building graph:", error);
